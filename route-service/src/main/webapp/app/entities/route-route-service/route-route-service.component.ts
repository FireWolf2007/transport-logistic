...
@Component({
    selector: 'jhi-route-route-service',
    templateUrl: './route-route-service.component.html'
})
export class RouteRouteServiceComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.routeService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<RouteRouteService[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/route-route-service'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/route-route-service', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: RouteRouteService) {
        return item.id;
    }
